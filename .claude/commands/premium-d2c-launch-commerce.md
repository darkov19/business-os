---
description: Evaluate premium D2C ecommerce launch readiness using the Business OS overlay.
argument-hint: "[premium D2C brand or launch context]"
disable-model-invocation: true
---

Use the Business OS `Premium D2C Launch Commerce` overlay for:

$ARGUMENTS

Operating rules:

1. Use the canonical Business OS docs from `/home/darko/Code/chitr/docs`.
2. Load and follow:
   - `/home/darko/Code/chitr/docs/business-os-module-spec-o16-premium-d2c-launch-commerce.md`
   - `/home/darko/Code/chitr/docs/business-os-workflow-launch-product-or-service.md`
   - `/home/darko/Code/chitr/docs/business-os-artifact-templates.md`
   - `/home/darko/Code/chitr/docs/business-os-evidence-rubric.md`
   - `/home/darko/Code/chitr/docs/business-os-question-bank.md`
3. Keep the analysis tied to conversion decisions, not brand aesthetics alone.
4. Do not recommend paid scale if the commerce layer is not yet believable.
5. Prefer assisted or staged launch when premium trust and proof are still thin.

Aim to produce or outline:
- Premium D2C Launch Commerce Note
- PDP asset checklist where useful
- launch trust-gap note where useful
- assisted-selling recommendation note where useful
